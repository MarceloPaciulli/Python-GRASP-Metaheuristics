...
"""
#principal generador de inicial
def generating_initial():
	init_factible = []
	for i in range(1000):
...
#incorporo esos posibles elementos a la solución parcial
#retorna lista de todas las soluciones parciales
#con los 'aportes' de cada elemento según posición en columna (lista de listas)     
def incorp_elems_a_sol_parcial(e,candidatos):
	all_solutions = []
	soluciones = []
	aux = e.copy()
	for i in range(e.shape[0]):
		for j in range(e.shape[1]):
			aux[:,j][:,np.newaxis]=candidatos[i]
			soluciones.append(aux)
			aux=e.copy()
		all_solutions.append(soluciones)
		soluciones = []
