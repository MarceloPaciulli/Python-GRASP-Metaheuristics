...
#ganancias = np.loadtxt('ganancias_gap.dat')
#capacidades = np.loadtxt('capacidades_gap.dat')
#capacities = np.loadtxt('capacities_gap.dat')
#ganancias.resize(10,40)
#capacidades.resize(10,40)

#ACÁ DEFINO LOS DATOS CON LOS QUE TRABAJO
#TAMBIÉN EN func_greedy
...
#generación de solución inicial (no factible-experimental)
...
def func_evaluacion(X):
        ...
        if output_1[0] & output_2:
                #print(benefit(X_1))
                salida = sum_costs(X_1)
        else:
                salida = - sum_costs(X_1) - output_1[1]
        return salida 
